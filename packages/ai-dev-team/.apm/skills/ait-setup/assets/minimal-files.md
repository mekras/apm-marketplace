...
Перед ответом, вопросом, исполнением или передачей работы по каждому элементу
корневого входа открой `<путь к установленному ait-routing/SKILL.md>`. Если этот
путь недоступен, используй `<запасной путь для второго клиента>`. Первым
сообщением покажи маршрут со строками «Режим менеджера» и «Маршрут».
