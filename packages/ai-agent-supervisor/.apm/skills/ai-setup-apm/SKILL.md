...
физические деревья и `apm.lock.yaml` через `validate-python-artifacts.py`. После
`apm install --frozen` дополнительно проверяй неизменность семантического
lock-графа. При изменении графа останавливай цикл до аудита и сообщай об
изменении семантики инструмента.
...
Указание валидатора — детерминированное правило границы, а не готовое решение о
судьбе файла. Если та же проекция служит артефактом, который внешний инструмент
читает при запуске сеанса, снятие отслеживания лишает свежий клон этого
артефакта до первой установки. В таком случае примени порядок «Потребители и
жизненный цикл артефакта» из `ai-data-collection`, назови конфликт границы и
доступности явно и оставь выбор человеку. Молча снимать отслеживание нельзя.
